...
`CER & Route Recommendation` es ahora la seccion principal de routing. El usuario elige una politica (`Minimum Time`, `Safety + Time`, `CER-Cost` o `CER-Agility`) y el workbench deriva el solver interno: Dijkstra para minimo coste/CER-Cost y Yen para CER-Agility. `Cost base` esta fijado a `minimum_travel_time`: el peso base de arista es tiempo de viaje, y escaleras/rampas/ascensores ya penalizan porque su factor de velocidad es menor.
