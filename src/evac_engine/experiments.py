...
BUILTIN_ROUTING_PRESETS: dict[str, dict[str, Any]] = {
    ...
    "cer_weighted": {
        "presetId": "cer_weighted",
        "label": "CER-Cost",
        "description": "Politica CER-Cost: minimiza tiempo + seguridad + penalizacion inversa por baja CER estructural.",
        "routing": {
            "algorithm": "dijkstra",
            "costPolicy": "minimum_travel_time",
            "useHazardRisk": True,
    ...
    "cer_agility_yen": {
        "presetId": "cer_agility_yen",
        "label": "CER-Agility",
        "description": "Politica CER-Agility: genera candidatas con Yen y selecciona la ruta que atraviesa nodos con mayor CER.",
        "routing": {
            "algorithm": "yen_ksp",
            "costPolicy": "minimum_travel_time",
            "useHazardRisk": True,
...
def _summarize_run(
    ...
    row = {
        ...
        "meanReroutingAgility": _mean(item.get("reroutingAgility") for item in route_metrics),
        "meanSelectionScore": _mean(item.get("selectionScore") for item in route_metrics),
        "meanCerAdjustedCost": _mean(item.get("cerAdjustedCost") for item in route_metrics),
        ...
        rows.append(
            {
                ...
                "reroutingAgility": metrics.get("reroutingAgility"),
                "selectionScore": metrics.get("selectionScore"),
                "cerAdjustedCost": metrics.get("cerAdjustedCost"),
